...
public class ECCBenchmark {
    ...
    public static void main(String[] args) {
        ...
        /**
         * Testet die Ver- und Entschlüsselungszeit
         */
        //testEncDec(iterationen, testTextEinfach, testTextEinfachChiffrat);

        /**
         * Testet die Parameter Generierung
         */
        testParamGen(iterationen, bitLength, millerRabin);

    }

    private static void testEncDec(int iterationen, String text, String textChiffrat) {
    ...
    private static void testParamGen(int iterationen, int bitlength, int millerRabin) {

        List<Long> keyGenTime = new ArrayList<Long>();

        for (int i = 0; i < iterationen; i++) {
            // Parameter-Generierung
            long start = System.currentTimeMillis();
            api.generateKeysAndParameters(bitlength, millerRabin);
            long end = System.currentTimeMillis();
            keyGenTime.add(end - start);

            System.out.println(i);
        }

        double avgenc = keyGenTime.stream().mapToLong(Long::longValue).average().orElse(0);

        double stdEnc = Math.sqrt(keyGenTime.stream()
                .mapToDouble(t -> Math.pow(t - avgenc, 2)).average().orElse(0));

        System.out.println("=== Ergebnis bei 256 Bit ===");
        System.out.println("Avg ParameterGenerationTime: " + avgenc + "ms (Standardabweichung: " + stdEnc + ")");
    }
